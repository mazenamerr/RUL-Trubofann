...
model_path = os.path.join(DATA_DIR, f"transformer_rul_capped_seq{SEQ_LEN}.pt")
...
eval_csv_path = os.path.join(DATA_DIR, f"test_eval_transformer_capped_seq{SEQ_LEN}.csv")
worst_csv_path = os.path.join(DATA_DIR, f"worst_10_transformer_capped_seq{SEQ_LEN}.csv")
final_out = os.path.join(DATA_DIR, f"final_test_results_transformer_capped_seq{SEQ_LEN}.csv")
...
plt.title("Predicted vs True RUL (Transf. Capped Model)", fontweight="bold")
plt.text(
...
plt.legend()
scatter_path = os.path.join(plots_transformer_dir, f"true_vs_pred_scatter_transf_capped_seq{SEQ_LEN}.png")
...
plt.title("Distribution of Pred. Err. (Transf. Capped Model)", fontweight="bold")
...
hist_path = os.path.join(plots_transformer_dir, f"error_histogram_transf_capped_seq{SEQ_LEN}.png")
...
plt.title("Absolute Error vs True RUL (Transf. Capped Model)", fontweight="bold")
plt.axhline(pred_df["abs_err"].mean(), linestyle="--", color='r', linewidth=1, label="MAE level")
plt.legend()
abs_err_path = os.path.join(plots_transformer_dir, f"abs_error_vs_true_transf_capped_seq{SEQ_LEN}.png")
...
plt.title("CDF of Absolute Pred. Err. (Transf. Capped Model)", fontweight="bold")
...
plt.legend()
cdf_path = os.path.join(plots_transformer_dir, f"cdf_abs_error_transf_capped_seq{SEQ_LEN}.png")
