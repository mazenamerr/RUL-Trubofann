...
model_out_path = os.path.join(DATA_DIR, f"cnn_lstm_rul_capped_seq{SEQ_LEN}.pt")
...
y_train = np.clip(y_train, 0, RUL_CAP)
y_val   = np.clip(y_val, 0, RUL_CAP)
...
# ---------- 4) Weights (same as Transformer) ----------
def make_weights(y: np.ndarray, cap: float = 150.0):
    y = y.astype(np.float32)
    w = 1.0 + (cap - y) / cap
    return np.clip(w, 1.0, 2.0).astype(np.float32)

w_train = make_weights(y_train, cap=RUL_CAP)
...
# Optimizer / scheduler
optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=1e-4)  # often helps RNNs vs 1e-4
...
for epoch in range(1, EPOCHS + 1):
    ...
    for xb, yb, wb in train_loader:
        ...
        optimizer.zero_grad()
        pred = torch.clamp(model(xb), 0.0, RUL_CAP)
        ...
        train_rmse_sum += rmse(pred, yb).item() * bs
        train_mae_sum  += mae(pred, yb).item() * bs
        n_train += bs
