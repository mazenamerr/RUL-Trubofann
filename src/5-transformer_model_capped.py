...
LR, D_MODEL, NHEAD, NUM_LAYERS, DROPOUT = 5e-4, 32, 4, 1, 0.2
...
model_out_path = os.path.join(DATA_DIR, f"transformer_rul_capped_seq{SEQ_LEN}.pt")
...
def make_weights(y: np.ndarray, cap: float = 125.0):
...
class TransformerRUL(nn.Module):
    ...
    def forward(self, x):
        ...
        # Attention pooling over time (learned importance for each timestep)
